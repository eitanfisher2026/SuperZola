          ...
          <div className="space-y-2 mb-3">
            {candidates.list.length === 0 ? (
              <div className="text-center py-6">
                ...
                <p className="text-[#A79A7C] text-xs mt-1.5">
                  {/[A-Za-z]/.test(searchQuery || draft.name || "") ? "נסו בעברית, או רק אותיות ראשונות של שם המותג" : "נסו רק אותיות ראשונות של שם המותג"}
                </p>
              </div>
        ...
        {tab === "setup" ? (
          <React.Fragment>
            ...
            <HelpCard icon="🛒" title="3. רוצים גם מחירי אונליין?">
              ...
              <br /><br />
              קונים רק בחנות, או רק אונליין, ולא רוצים לראות את הצד השני בכלל? לחצו על גלגל השיניים ⚙️ — הפריט הראשון בתפריט הוא בדיוק בשביל זה: "בחנות בלבד" / "גם וגם" / "אונליין בלבד". הבחירה חלה על כל הרשימות שלכם ואפשר לשנות אותה בכל רגע.
            </HelpCard>
            ...
            <HelpCard icon="🔀" title="8. תצוגת בחנות מול אונליין">
              כשההגדרה האישית שלכם (⚙️ ← הפריט הראשון בתפריט) היא "גם וגם", וגם יש לכם סניפים פיזיים וגם רשתות אונליין פעילים, מופיע מתג נוסף בראש הרשימה — "בחנות" / "אונליין" — שמחליף אילו מחירים מוצגים לאותם הפריטים בדיוק. אם ההגדרה היא "בחנות בלבד" או "אונליין בלבד", המתג לא מופיע בכלל — הרשימה פשוט מציגה תמיד את הצד שבחרתם. השוואת העלות הכוללת בין השתיים נמצאת באופטימיזציית הקניות (☰).
            </HelpCard>
          </React.Fragment>
        ) : (
          <React.Fragment>
            ...
            <HelpCard icon="🏪" title="רשתות מוצגות">
              מסתירים רשת מסוימת רק ברשימה הזו, בלי לכבות אותה לגמרי — שימושי כשלא מתכננים לקנות שם הפעם. הסניפים הפיזיים ורשתות האונליין מוצגים בשתי קבוצות נפרדות. מאותו מסך אפשר גם להוסיף סניף חדש (לא רק לנהל את הקיימים). זה שונה מההגדרה "בחנות בלבד / גם וגם / אונליין בלבד" בתפריט ⚙️ — זו חלה על כל הרשימות ביחד, בעוד ההסתרה כאן היא רק לרשימה הנוכחית.
            </HelpCard>
